...
def dump_memory_profile(filename_suffix='runtime'):
  ...
  try:
    ...
    # Fall back to reading /proc on Linux
    try:
  ...
  with open(snapshot_filename, 'w') as f:
    f.write(f'Memory Profile Dump ({filename_suffix})\n')
    f.write('=' * 80 + '\n')
    f.write(f'Timestamp: {timestamp}\n')
    f.write(f'PID: {pid}\n')
    f.write(f'Elapsed time: {elapsed_minutes} minutes\n')
    if rss_mb is not None and vms_mb is not None:
      f.write(f'Process memory usage: RSS={rss_mb:.1f} MiB, VMS={vms_mb:.1f} MiB\n')
    f.write('\n')
